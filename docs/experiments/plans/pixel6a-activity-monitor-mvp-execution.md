...
- 基本フローは `Termux収集 -> Sheets追記 -> 判定 -> Slack通知`。
- 各フェーズを1 Issue で閉じる。
...
4. Issue 04: Slack通知ジョブを実装し、通知レート制御を入れる。
...
### Issue 04: Slack Notifier

- Scope:
  - 判定結果をSlackに送信する。
  - 同一時間窓の重複通知を抑止する。
- Acceptance Criteria:
  - 通知本文に期間・活動レベル・距離が含まれる。
  - API失敗時にリトライしても重複しない。
- Verification:
  - 追加する `scripts/ci/test-slack-notify.sh`
...
- Scope:
  ...
  - ログ形式・保存先・保持期間をMVP運用として固定する。
- Acceptance Criteria:
  - 収集・判定・通知ジョブが定期実行される。
  - 失敗時に復旧手順で再開できる。
  - Tailscale接続断時の確認手順と復旧手順がrunbookに明記されている。
  - ログ方針に沿って機微情報非表示と短期保持が実装されている。
- Verification:
...
- 目的:
  - 障害調査と復旧判断に限定する。
- 形式:
  - 1行JSONログ (`ts`, `level`, `job`, `event`, `result`, `error_code`, `retry_count`, `tailnet_ok`)。
- 出力禁止:
